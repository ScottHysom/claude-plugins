...
class DescribeSegments:
    ...
    @pytest.mark.spec("comment-cut-out")
    ...
    @pytest.mark.spec("comment-cut-out")
    def it_gives_nothing_for_the_spaces_a_comment_leaves_on_one_side(self):
        segs = segments("Text <!-- a note --> \n")
        assert [(s["col_start"], s["col_end"], s["text"]) for s in segs] == [(0, 4, "Text")]
...
class DescribeTheSegmentsCommand:
    @pytest.mark.spec("segment-lines")
    ...
    @pytest.mark.spec("scope-by-default")
    def it_names_a_file_given_that_does_not_exist(self, prose_repo, capsys):
        code, out, err = run_text(prose_repo, capsys, "target.md", "missing.md")
