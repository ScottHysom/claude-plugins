...
Commands:
    ...
    selftest    in-memory round-trip checks
...
class Text:
    ...
    missing final newline. selftest asserts that.
    """
...
# selftest
#
# This repository has no test harness and does not want one. The property that
# matters is a round trip: for any batch of insertions, insert then strip must
# return the file byte-identical. That is checkable in memory, with no
# fixtures, and it is one line in CI.
# --------------------------------------------------------------------------
...
def _round_trip(records, source=SAMPLE):
    text = Text(source)
    blocks = Blocks(text)
    engine, refusals, _ = apply_inserts(text, blocks, records, "sample.md", 1)
    if refusals:
        return "refused: %s" % refusals[0]
    tagged = Text(engine.result())
    scanner = TagScanner(tagged, None, "sample.md")
    if scanner.errors:
        return "tagged text does not parse: %s" % scanner.errors[0]
    back, _ = resolve_text(tagged, REJECT, None, "sample.md")
    if back != source:
        return "strip did not restore the original"
    return None
...
def selftest_cases():
    ...
    def text_round_trip():
        for s in ["", "a", "a\n", "a\nb", "a\r\nb\r\n", "no trailing newline",
            ...
            t = Text(s)
            if "".join(t.lines) != s:
                return "lines do not rejoin for %r" % s[:20]
        return None
    case("Text preserves bytes", text_round_trip)

    def globs():
        checks = [("**/*.md", "a.md", True), ("**/*.md", "x/y/a.md", True),
                  ("*.md", "x/a.md", False), ("skills/**", "skills/a/b.md", True),
                  ("skills/**", "skill/a.md", False),
                  ("**/README.md", "README.md", True),
                  ("**/README.md", "docs/README.md", True),
                  ("prose-style.md", "a/prose-style.md", False)]
        for pat, path, want in checks:
            if bool(glob_to_regex(pat).match(path)) != want:
                return "%s vs %s should be %s" % (pat, path, want)
        return None
    case("glob translation", globs)

    def fence_shield():
        scanner = TagScanner(Text(SAMPLE), None, "sample.md")
        if scanner.all:
            return "found markup inside a code fence"
        return None
    ...
    case("inline del round trip", inline_round_trip)
    ...
    case("inline repl round trip", repl_round_trip)
    ...
    case("batch round trip", batch_round_trip)

    def accept_semantics():
        src = ('Keep <del>cut this</del><ins>keep this</ins> here.\n'
               '<repl why="noun phrase"><del>old</del><ins>new</ins></repl>\n')
        text = Text(src)
        got, scanner = resolve_text(text, ACCEPT, None, "t.md")
        if scanner.errors:
            return "parse errors: %s" % scanner.errors[0]
        if got != "Keep keep this here.\nnew\n":
            return "accept produced %r" % got
        back, _ = resolve_text(text, REJECT, None, "t.md")
        if back != "Keep cut this here.\nold\n":
            return "reject produced %r" % back
        return None
    case("accept and reject semantics", accept_semantics)

    def pair_detection():
        src = "<del>a</del><ins>b</ins>\n"
        scanner = TagScanner(Text(src), None, "t.md")
        pairs, _ = scanner.pairs()
        if len(pairs) != 1:
            return "bare del+ins pair not detected"
        return None
    case("bare pair is a replacement", pair_detection)
    ...
    def grammar_errors():
        checks = [
            ...
            ("<repl><del>old</del></repl>\n", "exactly one"),
            ("<del bogus=\"x\">a</del>\n", "no 'bogus' attribute"),
            ("<ins><del>no</del></ins>\n", "not allowed inside"),
        ]
        for src, want in checks:
            scanner = TagScanner(Text(src), None, "t.md")
            if not any(want in e for e in scanner.errors):
                return "%r should report %r, got %s" % (src, want,
                                                        scanner.errors)
        return None
    case("malformed markup is caught", grammar_errors)

    def one_fault_one_message():
        """Pain point 4: malformed tags drifted in and nothing caught them.

        Each shape must report exactly one line. A parser that says two things
        about one typo teaches the author to skim its output.
        """
        shapes = [
            "<del>text with no close\n",
            "text\n</del>\n",
            "<del>text</ins>\n",
            "<del>text</de>\n",
            '<del reason="x">text</del>\n',
            "<ins><del>no</del></ins>\n",
            "<repl><del>old</del></repl>\n",
            "<repl><ins>new</ins><del>old</del></repl>\n",
            '<q id="1">a</q>\n<q id="1">b</q>\n',
        ...
        for src in shapes:
            errs = TagScanner(Text(src), None, "t.md").errors
            if len(errs) != 1:
                return "%r reported %d errors: %s" % (src, len(errs), errs)
        return None
    case("one fault, one message", one_fault_one_message)

    def signals():
        checks = [("we saw 3 things", "we saw 4 things", "numeric-only"),
                  ("a  b", "a b", "whitespace-only"),
                  ("see [x](/a)", "see [x](/b)", "link-only"),
                  ("short line", "a longer line", None)]
        for old, new, want in checks:
            if classify_signal(old, new) != want:
                return "%r -> %r gave %s" % (old, new,
                                             classify_signal(old, new))
        return None
    case("signal classification", signals)
    ...
    def config_parse():
        ...
        try:
            ...
            os.unlink(path)
        if len(cfg.rules) != 2:
            return "expected 2 rules, got %d" % len(cfg.rules)
        if cfg.scope_include != ["**/*.md"] or cfg.scope_exclude != ["x.md"]:
            return "scope block not parsed"
        if not any("duplicate rule id" in e for e in cfg.errors):
    ...
    def front_matter_grammar():
        ...
        try:
            ...
            os.unlink(path)
        if not cfg.errors:
            return "out-of-grammar front matter was accepted silently"
        return None
    case("front matter grammar is enforced", front_matter_grammar)

    def refusals():
        text, blocks = Text(SAMPLE), Blocks(Text(SAMPLE))
        bad = [
            ({"kind": "del", "start": 5}, "heading"),
            ({"kind": "del", "start": 19}, "fence"),
            ({"kind": "del", "start": 14}, "table"),
            ({"kind": "del", "start": 7, "col_start": 5, "end": 8,
              "col_end": 3}, "straddles"),
        ]
        for rec, want in bad:
            try:
                plan_one_insert(text, blocks, rec, "sample.md", 1)
            except InsertRefusal as exc:
                if want not in str(exc):
                    return "%s refused with %r, wanted %r" % (rec, exc, want)
                continue
            return "%s was not refused" % rec
        return None
    case("unsafe spans are refused", refusals)

    def segment_protection():
        text = Text(SAMPLE)
        segs = segments_for(text, Blocks(text))
        for seg in segs:
            if "x = 1" in seg["text"] or "title: sample" in seg["text"]:
                return "a protected line was emitted as a segment"
        if not any(s["kind"] == "table-cell" for s in segs):
            return "table cells were not emitted"
        if not any(s["kind"] == "heading" for s in segs):
            return "headings were not emitted"
        return None
    case("segments skip protected regions", segment_protection)
...
def cmd_selftest(args):
    ...
    for name, fn in cases:
        try:
            problem = fn()
        except Exception as exc:  # a crash is a failure like any other
            problem = "%s: %s" % (type(exc).__name__, exc)
        if problem:
            failures.append("FAIL %s: %s" % (name, problem))
    for line in failures:
        sys.stderr.write(line + "\n")
    print("%d/%d selftest case(s) passed"
          % (len(cases) - len(failures), len(cases)))
    return PROBLEMS if failures else OK
...
def build_parser():
    ...
    p = sub.add_parser("selftest", parents=[common],
                       help="in-memory round-trip checks")
